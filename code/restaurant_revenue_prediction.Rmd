---
title: "Kaggle - Restaurant Revenue Prediction (or: Regression with little training data)"
...
```{r load, cache=TRUE}
# get train data from https://www.kaggle.com/c/restaurant-revenue-prediction/download/train.csv.zip
train <- read.csv("../input/train.csv", stringsAsFactors=F)
# get test data from https://www.kaggle.com/c/restaurant-revenue-prediction/download/test.csv.zip
...
library(caret)
train[train==0] = NA
tempData <- mice(train, m=1, maxit=2, meth='rf', seed=501, printFlag=FALSE)
...
Result: nothing to remove due to nera-zero variance.
...
- This plot shows that there is some correlation between numerical predictors
    ...
    + There is unfortunately very little correlation between predictors and revenue,
    so that we don't expect well-performing models.
...
I tested many different models and tuned them using the caret package.
...
```{r form, warning=FALSE, cache=TRUE}
library(caret)
library(mice)
...
# get example submission from https://www.kaggle.com/c/restaurant-revenue-prediction/download/sampleSubmission.csv
...
impute = function(data, method='rf') {
    data[data==0] = NA
    tempData <- mice(data, m=1, maxit=2, meth=method, seed=501, printFlag=FALSE)
    # second parameter is the number of the dataset. We have m datasets.
...
test = read.data('../input/test.csv')
prepare.submission(gbmFit, test, 'gbm_ni_nt_nc_nf') # Leaderboard 1844142.14327
leaderboard = data.frame(fit='gbm_ni_nt_nc_nf', rmse=1844142.14327, position=770)
```

One can see that the $R^2$ value is very low, as expected due to the small training dataset.
The line `leaderboard = ...` indicate the RMSE on the private Kaggle leaderboard as well as the position achieved ou of 2257 competing teams.
...
leaderboard = rbind(leaderboard, data.frame(fit='gbm_it_nt_nc_nf', rmse=1845526.06265, position=785))
```
...
leaderboard = rbind(leaderboard, data.frame(fit='gbm_it_nt_rc_nf', rmse=1845879.34674, position=787))
```
...
leaderboard = rbind(leaderboard, data.frame(fit='gbm_it_nt_nc_wf', rmse=1832814.59420, position=618))
```
...
leaderboard = rbind(leaderboard, data.frame(fit='gbm_it_wlt_nc_wf', rmse=1780045.02033, position=57))
```
...
leaderboard = rbind(leaderboard, data.frame(fit='gbm_itt_wlt_nc_wf', rmse=1785557.44521, position=79))
```
...
test = add.features(test)
train[train==0] = NA
tempData <- mice(train, m=NUM.IMPUTATIONS, maxit=2, meth='rf', seed=501)
...
write.csv(result, file = '../output/gbm_mi_wlt_nc_wf_results.csv',
          quote = FALSE, row.names = FALSE)
leaderboard = rbind(leaderboard, data.frame(fit='gbm_mi_wlt_nc_wf', rmse=1953404.65481, position=1725))
```
...
write.csv(result, file = '../output/gbm_mi_nt_nc_wf_results.csv',
          quote = FALSE, row.names = FALSE)
leaderboard = rbind(leaderboard, data.frame(fit='gbm_mi_nt_nc_wf', rmse=1796661.54897, position=146))
```
